    ...
    return (
        <div className="max-w-xl mx-auto space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6 space-y-3">
                <label className="text-sm text-white/60">Tipe Topup</label>
                <div className="grid grid-cols-2 gap-3">
                    <button
                        onClick={() => setSource("cash")}
                        className={`p-4 rounded-xl border-2 transition-all group ${source === "cash"
                            ? "border-blue-500 bg-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.1)]"
                            : "border-white/5 bg-white/5 hover:bg-white/10 hover:border-white/10"
                            }`}
                    >
                        <DollarSign className={`w-8 h-8 mx-auto mb-2 transition-colors ${source === "cash" ? "text-blue-400 drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]" : "text-white/30 group-hover:text-white/50"}`} />
                        <div className={`font-medium ${source === "cash" ? "text-blue-400" : "text-white/40 group-hover:text-white/70"}`}>Cash</div>
                        <p className="text-xs text-white/30 mt-1">Bayar langsung</p>
                    </button>
                    <button
                        onClick={() => setSource("gift")}
                        className={`p-4 rounded-xl border-2 transition-all group ${source === "gift"
                            ? "border-pink-500 bg-pink-500/10 shadow-[0_0_20px_rgba(236,72,153,0.1)]"
                            : "border-white/5 bg-white/5 hover:bg-white/10 hover:border-white/10"
                            }`}
                    >
                        <Gift className={`w-8 h-8 mx-auto mb-2 transition-colors ${source === "gift" ? "text-pink-400 drop-shadow-[0_0_8px_rgba(244,114,182,0.5)]" : "text-white/30 group-hover:text-white/50"}`} />
                        <div className={`font-medium ${source === "gift" ? "text-pink-400" : "text-white/40 group-hover:text-white/70"}`}>Gift</div>
                        <p className="text-xs text-white/30 mt-1">Gratis/Hadiah</p>
                    </button>
                </div>
            </div>
            ...
            <div className="glass-card rounded-2xl p-6 space-y-4">
                <h3 className="text-sm font-medium text-white/60 uppercase tracking-wider">Informasi Transaksi</h3>

                <div className="space-y-2">
                    <label className="text-sm text-white/50">SKU Product</label>
                    <input
                        type="text"
                        value={sku}
                    ...
                    <p className="text-xs text-white/30">
                        Pastikan SKU valid (Cek di <Link href="/admin/products" className="text-primary hover:text-primary/80 hover:underline">products list</Link>)
                    </p>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <input
                        ...
                        className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary/50 placeholder:text-white/20 transition-all"
                    />
                </div>

                <div className="space-y-2">
                    <label className="text-sm text-white/50">Catatan (Opsional)</label>
                    <input
                        type="text"
                        value={notes}
            ...
            <button
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg ${source === "cash"
            ...
            >
                <div className="space-y-6">
                    ...
                    {/* 4. Notes Display */}
                    {notes && (
                        <div className="text-sm p-3 bg-white/5 rounded-xl text-white/80 border border-white/5">
                            <span className="text-white/40 text-xs block mb-1">Catatan:</span>
                            {notes}
                        </div>
                    )}
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    maxLength={6}
                                    className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-3 text-white text-sm font-mono tracking-widest text-center focus:outline-none focus:border-primary/50 placeholder:text-white/20 transition-colors"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-white font-medium text-sm transition-colors flex items-center justify-center gap-2 shadow-lg ${source === "cash"
                                ? "bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 shadow-blue-900/20"
                                : "bg-pink-600 hover:bg-pink-500 disabled:bg-pink-600/50 shadow-pink-900/20"
                                }`}
                        >
